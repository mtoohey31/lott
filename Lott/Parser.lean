...
syntax " | " prodArg+ " : " withPosition(ident (lineEq "nosubst")? "notex"?) atomic(bindConfig)? atomic(idConfig)? atomic(expandConfig)? (texConfig)? : Lott.Production
...
syntax "nosubst"? "nonterminal " atomic("(" parent " := " ident ")")? (texPrePostConfig)? ident,+ " := " Lott.Production* : Lott.NonTerminal
...
syntax "judgement_syntax " stx+ " : " ident atomic(idConfig)? (texConfig)? : command

private
def bracketedBinder := Term.bracketedBinder

syntax withPosition(Lott.Judgement)* "─"+ withPosition(ident (lineEq bracketedBinder)* (lineEq "notex")?) withPosition(Lott.Judgement) : Lott.InferenceRule
